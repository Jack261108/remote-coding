...
class TestRefreshSeenMtimes:
    async def test_detects_new_files(self, tmp_path):
        f = tmp_path / "test.txt"
        f.write_text("hello")
        seen: dict[str, float] = {}

        updated = await refresh_seen_mtimes({str(f)}, seen)

        assert str(f) in updated
        assert seen[str(f)] == updated[str(f)]

    async def test_detects_modified_files(self, tmp_path):
        f = tmp_path / "test.txt"
        f.write_text("hello")
        seen: dict[str, float] = {}
    ...
    async def test_skips_missing_files(self, tmp_path):
        seen: dict[str, float] = {}
        updated = await refresh_seen_mtimes({str(tmp_path / "missing.txt")}, seen)
        assert updated == {}

    async def test_handles_multiple_paths(self, tmp_path):
        f1 = tmp_path / "a.txt"
        f2 = tmp_path / "b.txt"
        f1.write_text("a")
        f2.write_text("b")
        ...
        updated = await refresh_seen_mtimes({str(f1), str(f2)}, seen)
