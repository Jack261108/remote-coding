...
class TestRefreshSeenMtimes:
    def test_new_file_detected(self, tmp_path) -> None:
        ...
        f.write_text("hello")
        seen: dict[str, float] = {}
        result = refresh_seen_mtimes({str(f)}, seen)
        assert str(f) in result
        assert seen[str(f)] == result[str(f)]

    def test_unchanged_file_not_in_result(self, tmp_path) -> None:
        f = tmp_path / "file.txt"
        f.write_text("hello")
        seen: dict[str, float] = {}
    ...
    def test_modified_file_detected(self, tmp_path) -> None:
        f = tmp_path / "file.txt"
        f.write_text("v1")
        seen: dict[str, float] = {}
        refresh_seen_mtimes({str(f)}, seen)
    ...
    def test_nonexistent_file_skipped(self, tmp_path) -> None:
        seen: dict[str, float] = {}
        result = refresh_seen_mtimes({str(tmp_path / "nonexistent.txt")}, seen)
        assert result == {}
        assert str(tmp_path / "nonexistent.txt") not in seen

    def test_empty_paths(self) -> None:
        seen: dict[str, float] = {}
        result = refresh_seen_mtimes(set(), seen)
        assert result == {}
    ...
    def test_multiple_files(self, tmp_path) -> None:
        ...
        f2.write_text("b")
        seen: dict[str, float] = {}
        result = refresh_seen_mtimes({str(f1), str(f2)}, seen)
        assert len(result) == 2
        assert str(f1) in seen
        assert str(f2) in seen

    def test_mixed_existing_and_new(self, tmp_path) -> None:
        f1 = tmp_path / "old.txt"
        f2 = tmp_path / "new.txt"
        f1.write_text("old")
        f2.write_text("new")
        seen: dict[str, float] = {}
        refresh_seen_mtimes({str(f1)}, seen)
        result = refresh_seen_mtimes({str(f1), str(f2)}, seen)
        assert str(f1) not in result
        assert str(f2) in result

    def test_seen_mtimes_mutated_in_place(self, tmp_path) -> None:
        f = tmp_path / "file.txt"
...
class TestClearSeenMtimesForSession:
    def test_removes_matching_keys(self) -> None:
        seen = {
            "session_abc/file1.txt": 1.0,
            "session_abc/file2.txt": 2.0,
            "session_xyz/file3.txt": 3.0,
        }
        clear_seen_mtimes_for_session("session_abc", seen)
        assert "session_abc/file1.txt" not in seen
        assert "session_abc/file2.txt" not in seen
        assert "session_xyz/file3.txt" in seen

    def test_no_match_no_removal(self) -> None:
        seen = {"a.txt": 1.0, "b.txt": 2.0}
        clear_seen_mtimes_for_session("nonexistent", seen)
        assert len(seen) == 2

    def test_empty_cache(self) -> None:
        seen: dict[str, float] = {}
        clear_seen_mtimes_for_session("any", seen)  # should not raise

    def test_session_id_substring_match(self) -> None:
        seen = {
            "abc_session_xyz/file.txt": 1.0,
            "other/file.txt": 2.0,
        }
        clear_seen_mtimes_for_session("session", seen)
