...
当前 Telegram 端会把 Claude 输出通过 `ChunkSender` 分块发送。结构化会话已经能识别工具调用、权限请求和用户提问，但工具调用进度主要以普通进度消息形式发送，长任务中会产生较多过程消息，也不方便跟踪每个非交互工具的最终状态。
...
- 非交互工具成功后更新为最终状态消息并保留。
- 非交互工具失败或中断时保留状态消息，方便排查。
...
- `ToolMessageManager`：管理 Telegram 工具消息生命周期，包括发送、编辑和失败降级。
...
- 非交互工具 `SUCCESS`：产出成功状态，供 Telegram 层编辑或发送“执行完成”最终状态消息。
...
1. 收到工具状态且没有现有消息：发送一条工具状态消息并记录。
2. 收到工具状态且已有消息：编辑该消息文本。
3. 收到成功状态且已有消息：编辑为“执行完成”并保留。
4. 收到成功状态但没有现有消息：发送一条“执行完成”最终状态消息并记录。
5. 编辑失败时：尝试重新发送一条工具状态消息并替换记录。
6. 发送、编辑及降级都失败时：记录日志，不向上抛出，不中断任务流。
...
- 没有现有消息时收到 `SUCCESS`，发送“执行完成”最终状态消息。
